...
# ★ 新增：加载标题映射（从 titles.json 加载，用于拼接标题到内容中）
TITLES = {}
...
def main():
    print("正在构建搜索索引 (V6) - 深度清理标签，保留内容，并拼接标题...")
    search_index = []
    for root, _, files in os.walk(DATA_DIR):
        for file in files:
            ...
            with open(path, "r", encoding="utf-8-sig") as f:
                ...
                # 2. 【关键】去掉 XML 标签 (text -> text)
                content = re.sub(r"<(red|blue)>(.*?)", r"\2", content, flags=re.DOTALL)
