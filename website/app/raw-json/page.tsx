  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="mx-auto max-w-[1600px] px-4 py-6 md:px-8">
        ...
        {manifest && (
          <>
            <section className="mb-5 rounded-xl border border-purple-200 bg-purple-50 p-4 text-sm text-purple-900">
              文件内容来自 <strong>{manifest.repository}</strong> 的固定提交 <code className="break-all">{manifest.commit}</code>。清单保存文件大小和SHA-256；打开文件不会受后续分支更新影响。
            </section>
            <section className="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-4"><strong className="text-2xl">{formatBytes(manifest.bytes)}</strong><span className="mt-1 block text-xs text-slate-500">文件总量（仓库源）</span></div>
            ...
            <section className="mb-5 grid gap-2 rounded-xl border border-slate-200 bg-white p-3 lg:grid-cols-[minmax(260px,1fr)_auto_auto_minmax(170px,auto)]">
              <label className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} /><input type="search" value={query} onChange={event => changeFilter(() => setQuery(event.target.value))} placeholder="搜索文件名、目录、剧情ID或仓库路径" className="w-full rounded-lg border border-slate-300 py-2.5 pl-10 pr-3 outline-none focus:border-purple-500" /></label>
            ...
            <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
              {shown.map(item => (
                <article key={item.url} className="grid gap-2 border-b border-slate-100 p-3 last:border-b-0 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
                  <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong className="break-all text-sm">{item.name}</strong><span className="rounded bg-slate-100 px-2 py-0.5 text-[10px] font-bold uppercase">{item.game}</span><span className="rounded bg-slate-100 px-2 py-0.5 text-[10px] font-bold uppercase">{item.language}</span><span className="text-[10px] text-slate-500">{item.category}</span></div><p className="mt-1 break-all text-xs text-slate-500">{item.repositoryPath} · {formatBytes(item.bytes)} · SHA-256 {item.sha256.slice(0, 16)}…</p>{item.storyIds.length > 0 && <p className="mt-1 break-all text-[10px] text-purple-700">剧情：{item.storyIds.slice(0, 8).join(' · ')}{item.storyIds.length > 8 ? ` 等${item.storyIds.length}项` : ''}</p>}</div>
                  <a href={item.url} target="_blank" rel="noreferrer" download className="inline-flex items-center justify-center gap-2 rounded-lg bg-slate-900 px-3 py-2 text-xs font-bold text-white hover:bg-purple-700"><Download size={14} />打开 / 下载</a>
                </article>
              ))}
            </section>
