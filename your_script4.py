    ...
    try:
        ...
        for group_name in all_groups:
            ...
            for item in group:
                ...
                if 'narration' in item or 'progressFnarration' in item:
                    raw_text = item.get('narration') or item.get('progressFnarration')
                    if item.get('nameNarration'):
                        sticky_narration_name = item['nameNarration']
...
def find_target_jsons(root, txt_filename):
    ...
    支持的 TXT 命名模式：
    1. 简单模式: 420221.txt → 匹配所有 420221*.json
    2. 范围模式: 513901-09_0-9.txt → 匹配 story ID 513901~513909, section 0~9
    3. 下划线范围: 420221_1-4.txt → 匹配 420221-*.json 中 section 1~4
    """
    ...
    # ★ 模式 A: 513901-09_0-9.txt (story ID 范围 + section 范围)
    # 匹配: 数字开头-数字结尾_数字-数字.txt
    match_full_range = re.match(r'^(\d+)-(\d+)_(\d+)-(\d+)\.txt$', txt_filename)
    if match_full_range:
        id_start_str = match_full_range.group(1)  # "513901"
        id_end_suffix = match_full_range.group(2)  # "09"
        sec_start = int(match_full_range.group(3))  # 0
        sec_end = int(match_full_range.group(4))    # 9
        
        # 计算 ID 范围
        # "513901" + "09" → 前缀 "5139" + 后缀从 "01" 到 "09"
        prefix = id_start_str[:len(id_start_str) - len(id_end_suffix)]  # "5139"
        suffix_start = int(id_start_str[len(prefix):])  # 01
        suffix_end = int(id_end_suffix)  # 09
        ...
        target = []
        for jf in all_jsons:
            # 从 JSON 文件名提取 story ID 和 section
            # 例: 513902-2_ceiHF.json → id=513902, sec=2
    ...
    # ★ 模式 B: 420221_1-4.txt (单个 story ID + section 范围)
    ...
    # ★ 模式 C: 420221.txt (简单模式，匹配所有同 ID 的 json)
...
def main():
    print("🚀 启动【全模式匹配 + 分支检测版】全量重建脚本...")
    
    rebuilt_count = 0
    match_debug = []  # 调试用：记录匹配情况
    ...
    for rel_dir in TARGET_DIRS:
        ...
        for root, dirs, files in os.walk(abs_root):
            for f in files:
                ...
                # 备份
                if not os.path.exists(txt_path + '.bak'):
                ...
                if full_content:
                    ...
                    if re.match(r'^\d+-\d+_\d+-\d+\.txt$', f):
                        ...
                        print(f"  ✅{branch_tag} 范围匹配: {f} → {len(target_jsons)} JSONs [{json_list}{suffix}]")
                    elif has_branch:
                        print(f"  ✅🌿 重建(含分支): {f}")
                    else:
                        print(f"  ✅ 重建完成: {f} ({len(target_jsons)} jsons)")
    
    print_branch_report()
    
    print(f"\n✨ 全部完成。重建 {rebuilt_count} 个文件，"
